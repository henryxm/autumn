...
| `POST /destroy` | ApiRegression | 管理员：须先软删再硬销毁；未软删拒绝 |
...
| `RobotApiRegressionTest` | 5 | Hook/令牌/销毁/回调 URL |
...
| `RobotQuotaRegressionTest` | 2 | 软删释放配额；停用仍占配额 |
| `RobotSoftDeleteGateRegressionTest` | 1 | 软删>10 门禁；硬销毁后恢复创建 |

**基建**：`IntegrationTest` 使用 `@TestInstance(PER_CLASS)`，同类共享一次启动与管理员令牌；`IntegrationJson.assertCode` / `assertBusinessFailure` / `assertAuthFailure`。
...
- Hook 列表 / 更新 / 删除；令牌创建与 revoke；`delete`→`destroy`（管理员）；未软删 `destroy` 失败；Hook 内网回调拒绝  
...
```java
public class MyFeatureIntegrationTest extends IntegrationTest {
