...
> **PostgreSQL 专项**（DDL、元数据、`PostgresQuerySql`、迁移细节）仍以 **`AI_POSTGRESQL.md`** 为准；**通用跨库口径以本文为准**。
...
3. **推荐**业务侧 `*DaoSql` **`extends RuntimeSql`**，通过 **`quote`、`limitOne`、`likeContainsAny`、`columnValueInCommaSeparatedList`、`enabledTrueSqlLiteral`、`currentTimestamp`、`truncateTable`** 等拼装，保证与 `RoutingRuntimeSqlDialect` / `RuntimeSqlDialectRegistry` 一致。
...
- **`apply` / 自定义 `sqlSegment`** 中写死 **函数名**（如 `FIND_IN_SET`、`IFNULL`、`DATE_FORMAT`、`GROUP_CONCAT`、专有类型转换）
...
> **「一键升级」边界**：工程上可实现 **一键扫描报告（只读）** + **分阶段人工改造**；**不能**指望无人值守自动改完所有 SQL 语义（见 **`AI_UPGRADE.md` §3**）。
...
1. **阶段 0**：依赖与配置升级（**`AI_UPGRADE.md` §2**）。  
2. **阶段 1**：运行 **`scripts/autumn-dependency-scan.sh`**（或等价 `rg`/`grep`），生成 **命中清单**（含本节相关规则，见 **`AI_UPGRADE.md` §3.1**）。  
3. **阶段 2**：按模块/包 **分批**改 Provider 与 Wrapper，每批 **编译 + 核心用例回归**。  
4. **阶段 3**：在 **非 MySQL** 环境做一次全量冒烟（或 CI 矩阵）。  
5. **门禁**：合并前 Code Review 检查 **新增**代码不得再引入 §8.1 中的模式（与 **`AI_STANDARDS.md`** §12 一致）。
...
- 新增或调整 **`DatabaseType`**、方言实现或路由规则时：**同步更新本文 §2** 与 **`AI_INDEX.md`**。  
- **实现类索引**：`DatabaseHolder`、`RoutingRuntimeSqlDialect`、`RoutingRelationalTableOperations`、`RuntimeSqlDialectRegistry`、`RuntimeSql`。
...
| 主题 | 文档 |
|------|------|
| 分层、Dao 禁止项、Controller 纪律 | **`AI_STANDARDS.md`** §12～§13 |
| PostgreSQL DDL、元数据、`PostgresQuerySql` | **`AI_POSTGRESQL.md`** |
| 启动配置、profile、环境变量 | **`AI_BOOT.md`** §3、§8 |
| 升级清单、扫描脚本、「一键」边界 | **`AI_UPGRADE.md`** |
