...
| 14a | **账号认证 / 登录跳转** | 升级后执行 **刷新全部 JSON**（`POST /sys/config/refreshJson`）；按需配置 `ACCOUNT_AUTH_CONFIG.postLoginRedirect`；回归 SavedRequest、直接登录、OAuth 授权页互不影响。见 **`docs/AI_ACCOUNT_AUTH_CONFIG.md` §8**。 |
