...
| **`uuid`** 或 **`user`** | `String` / `Long` / `String` | 第二主键（业务键） | 表间关联、对外 ID、缓存、领域引用 | 与 `id` 混用；与另一种第二主键列并存 |

**第二主键选型（二选一，禁止并存）**：

| 场景 | 第二主键列 | 标记接口 | 说明 |
|------|------------|----------|------|
| **默认**（通用实体） | **`uuid`** | **`UuidBased`** / **`SnowBased`** | 随机生成；列名固定 `uuid` |
| **按用户唯一**（每用户一行） | **`user`** | **`UserBased`** | 存 `sys_user.uuid`；**禁止**再设 `uuid` 第二主键 |

非按用户唯一的表：可有**非唯一**的 `user` 外键列（所属用户），与 `uuid` 第二主键**同时存在**；该 `user` **不得** `isUnique = true`。
...
| 接口 | 第二主键列 | 类型 / 生成 | 实体示例 |
|------|------------|-------------|----------|
| **`IdBased`** | — | — | 所有双键实体须实现（含 `getId`/`setId`） |
...
### 3.4 非唯一的 `user` 外键（与 `uuid` 第二主键并存）

当表**不是**按用户唯一、仍使用 **`uuid`** 第二主键时，可增加 **`user`** 表示所属/调用主体（**§1.1**：可为 **`sys_user.uuid`** 或 **`bot_robot.uuid`**）：
...
- **按用户唯一**表用 **`UserBased` + `user`**，**禁止** `uuid` 第二主键；其它表 **`user`** 仅作非唯一外键时可与 **`uuid`** 并存。
...
- [ ] 实体具备自增 **`id`** 与唯一业务第二主键（**`uuid`** 或按用户唯一的 **`user`**，二选一）
- [ ] 通用表：**`UuidBased`** / **`SnowBased`**；按用户唯一表：**`UserBased`**（**禁止**同时有 `uuid` 第二主键）
- [ ] 第二主键列 **`isUnique = true`**，且**不**叠 `@Index`（非唯一 `user` 外键除外，其**不得** unique）
