...
- **实体扫描与注解建表、禁止常规 DDL `.sql`、Dao 仅经 Provider 写 SQL、gen/Pages/html/js 不可改、`statics`/`pages`/`Site`/`@PageAware` 等**：以 **`docs/AI_STANDARDS.md`** §8～§14 为强制约束，模板任务不得与之冲突。**代码生成端到端流程与三步开发节奏**见 **`docs/AI_CODEGEN.md`**。
