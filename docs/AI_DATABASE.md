...
- **第二主键选型**（**新开发**；存量见 **`docs/AI_DUAL_KEY.md` §1.2**）：默认唯一 **`uuid`**；**每真人一行**用 **`UserBased` + 唯一 `user`**；**用户维度即可**时可**不设第二主键**（**§3.4**）。业务 **`user`** 外键可存真人或机器人 **`uuid`**。见 **§3.3～§3.5**。
...
- `Wrapper` / `Page` 的排序或等值列参数为**未经过** `columnInWrapper` / `orderByColumnExpression` 的裸标识符，且该路径**未**依赖框架已统一处理的入口（如未使用 **`Query` / `BaseService#getPage`**）。**MP 2.x** 常见为 `setOrderByField` / `setDescs`；**Autumn 3.x / MP 3.x** 分页排序优先 **`Page#addOrder` + `OrderItem.withExpression`**（框架内已配合 `WrapperColumns`）。
...
**仓库脚本（分组体检）**：根目录执行 **`scripts/constraints-scan`**（只读、`rg` 必选），按 **实体命名 / 索引唯一 / Dao 注解 SQL / Wrapper / 分层 / DDL 文件名** 及 **A～G / H（Redis TTL、`expire` 可疑写法，见 `docs/REDIS_TTL_GUIDE.md`）** 等分组输出；规则与 **`docs/AI_STANDARDS.md`**、本文 §1～§5、§8、**`docs/REDIS_TTL_GUIDE.md`** 及 **`docs/AI_BOOT.md`** 表名约定等对读。详见脚本头部注释与环境变量 **`AUTUMN_SCAN_SKIP_GEN`** / **`AUTUMN_SCAN_SKIP_REDIS`** / **`AUTUMN_SCAN_EXTRA`**；仅检 Redis 时用 **`--redis-expire-only`**。**执行时机**：与 **`docs/AI_INDEX.md`** 一致，**按需**（用户或任务要求、CI、合并前自检），**不**作为每次改动的默认必跑项。
