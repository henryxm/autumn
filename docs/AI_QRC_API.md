...
Autumn POST 到 `webhook`（建票 payload 或 `qrc_client_grant.webhook`）：
...
| `X-Qrc-Event` | `qrc.scanned` 或 `qrc.authorized` |
| `X-Qrc-Timestamp` | 毫秒时间戳 |
| `X-Qrc-Signature` | HMAC-SHA256 hex（body 原文 + `secret`） |

### 5.1 扫码（`qrc.scanned`）

用户扫码未确认时触发（`ScanTicketService.scan` 成功后）。
...
```json
{
  ...
  "data": {
    ...
    "scannerBrief": {
      "displayName": "昵称",
      "icon": "/uploads/avatar.png"
    }
  }
}
```

### 5.2 确认授权（`qrc.authorized`）

用户确认后触发（`ClientGrantService.deliverOAuth`）。

- **经典 `clientId`**：签发经典 OAuth 授权码（`issueAuthCode`），换票走 `/oauth2/token`。
- **OPL 活跃 `appId`**：签发 OPL 授权码（`OpenCodeService.issue`），换票走 `/open/oauth2/token`（`ConnectLoginService`）。
...
| GET | `/client/oauth2/qrc/web/ticket/stream?uuid=` |
| GET | `/client/oauth2/qrc/web/ticket/status?uuid=` |
| POST | `/client/oauth2/qrc/web/ticket/cancel?uuid=` |
| POST | `/client/oauth2/qrc/web/inbound` |

同源开放 OPC 扫码（`oauth2_open` + `qrMode=as`）：

| 方法 | 路径 |
|------|------|
| POST | `/qrc/scanticket/web/ticket/create`（body 含 `type`/`id`） |
| GET | `/qrc/scanticket/web/ticket/stream?uuid=` |
| GET | `/qrc/scanticket/web/ticket/status?uuid=` |
| POST | `/open/oauth2/qrc/web/complete` |
...
**`qrc.authorized` 入站** 按 `credentialType` 自动调用 **`WebOauthLoginService`**（classic）或 **`ConnectLoginService`**（open），浏览器经 SSE 接收 `redirectUrl`（见 **`docs/AI_AUTH_SITE_ROLES.md` §2**）。  
前端 **`autumn-qrc-core.js`**（`mode: 'rp'` / open `as`）与配置见 **`docs/AI_AUTH_SITE_ROLES.md`**。
