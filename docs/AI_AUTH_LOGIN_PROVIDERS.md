...
前端：`AutumnQrc.createMethods({ mode: p.qrMode, type: p.credentialType, id: p.id })` 或 `AutumnQrc.mergeInto`；RP/AS 均订阅 SSE `{prefix}/ticket/stream`（`onerror`/watchdog 降级 `ticket/status`）；开放同源（`qrMode=as`）确认后 `POST /open/oauth2/qrc/web/complete`。
