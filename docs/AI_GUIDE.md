...
- **实体与库表**：框架扫描实体并依开关**自动建表/更新**；**禁止**常规 **`DDL .sql`** 初始化脚本（§8）。模块目录名 = 包段 = **表前缀**，**勿**作实体类名前缀（§9）。表/字段 **`comment`** 用 **半角 `:`** 分隔短标题与说明；**凡 `@Column(isUnique=true)` 的字段禁止再使用 `@Index`**（§10.2，无例外）；整型/布尔优先**基本类型 + 默认值**（§10）。**默认**每个实体须有自增 **`id`** + 业务第二主键（**`uuid`** 或按用户唯一的 **`user`** / **`UserBased`**，见 **`docs/AI_DUAL_KEY.md`** §1、§3.3）；关联与对外 ID **禁止**用 **`id`**（§10.4；SQL 见 **`docs/AI_DATABASE.md` §1.1**）。
