...
授权页展示的两项权限与 API 字段对应关系：

| 授权页文案 | 对应数据 | API 字段 |
|-----------|----------|----------|
| 查看你的基本账号信息（昵称、头像等） | 展示资料 | `nickname`、`icon`、`username` |
| 获取你的用户唯一标识，用于第三方登录关联 | 账号绑定 | `uuid` |

**当前实现说明**：

- 客户端默认 `scope=basic`
- 换 token 响应中 `scope` 固定为 `basic`
- `/oauth2/userInfo` **不按 scope 裁剪字段**；token 有效即返回完整 `UserProfile`

若未来需要按 scope 分级返回，需在 AS 侧扩展；对接方现阶段可按「一次 userInfo 拿齐全部字段」实现。
