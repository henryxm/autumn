package cn.org.autumn.table.relational.dialect.firebird;
...
/**
 * Firebird：元数据以 JDBC 为主；DDL 由 {@link AnsiDoubleQuotedDdlGenerator} 生成（Firebird 3+ 标识列）。
 */
public final class FirebirdRelationalSchemaSql extends AbstractFullNoopRelationalSchemaSql {

    public static final FirebirdRelationalSchemaSql INSTANCE = new FirebirdRelationalSchemaSql();

    public FirebirdRelationalSchemaSql() {
    ...
    public String createTable(Map<String, Map<TableInfo, List<ColumnInfo>>> map) {
        return AnsiDoubleQuotedDdlGenerator.createTable(AnsiDialect.FIREBIRD, map);
    }

    @Override
    public String addColumns(Map<String, Map<TableInfo, ColumnInfo>> map) {
        return AnsiDoubleQuotedDdlGenerator.addColumns(AnsiDialect.FIREBIRD, map);
    }

    @Override
    public String modifyColumn(Map<String, Map<TableInfo, ColumnInfo>> map) {
        return AnsiDoubleQuotedDdlGenerator.modifyColumn(AnsiDialect.FIREBIRD, map);
    }

    @Override
    public String dropColumn(Map<String, Map<TableInfo, String>> map) {
        return AnsiDoubleQuotedDdlGenerator.dropColumn(AnsiDialect.FIREBIRD, map);
    }

    @Override
    public String dropPrimaryKey(Map<String, Map<TableInfo, ColumnInfo>> map) {
        return AnsiDoubleQuotedDdlGenerator.dropPrimaryKey(AnsiDialect.FIREBIRD, map);
    }

    @Override
    public String dropIndex(Map<String, Map<TableInfo, Object>> map) throws NoSuchFieldException, IllegalAccessException {
        return AnsiDoubleQuotedDdlGenerator.dropIndex(AnsiDialect.FIREBIRD, map);
    }

    @Override
    public String addIndex(Map<String, Map<TableInfo, IndexInfo>> map) {
        return AnsiDoubleQuotedDdlGenerator.addIndex(AnsiDialect.FIREBIRD, map);
    }
}
