...
public class ViewTemplateSupport {
    ...
    /** {@code redirect:/}、{@code forward:/}、404 回落页及已写响应标记，不参与存在性校验。 */
    public boolean isSpecialViewName(String viewName) {
        if (StringUtils.isBlank(viewName)) {
            return true;
        }
        return viewName.startsWith("redirect:") || viewName.startsWith("forward:")
                || FALLBACK_404_VIEW.equals(viewName) || REQUEST_HANDLED.equals(viewName);
    }
    ...
    private boolean classpathTemplateExists(String templatePath) {
        ...
        if (StringUtils.isNotBlank(suffix) && !normalized.endsWith(suffix)) {
        ...
        for (String path : paths) {
            for (String candidate : new String[] {"templates/" + path, path}) {
