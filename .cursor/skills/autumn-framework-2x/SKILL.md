...
3. **修复**  
   - 对本次任务**范围内**可确定的违规：**直接改代码**（例如去掉与 `isUnique` 冲突的 `@Index`、Dao 内联 SQL 改为 Provider、Controller 去掉 Dao 注入等）。  
   - 超出当前任务或需产品决策的：在回复中**列为残留风险**并说明理由，勿静默忽略扫描命中。

4. **收尾**  
   - 改过相关文件后，在条件允许时 **再跑一次** `constraints-scan`，确认本次修改未引入新的可修复项。
