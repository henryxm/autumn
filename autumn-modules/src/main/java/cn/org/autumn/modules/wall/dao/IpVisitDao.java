...
public interface IpVisitDao extends BaseMapper<IpVisitEntity> {
    ...
    @Select("update wall_ip_visit set `count` = ifnull(`count`,0) + #{count}, `user_agent` = #{userAgent}, `host` = #{host}, today = ifnull(today,0) + #{count}, update_time = now() where ip = #{ip}")
    Integer count(@Param("ip") String ip, @Param("userAgent") String userAgent, @Param("host") String host, @Param("count") Integer count);
