...
public class OplAdminService {
    ...
    public Map<String, Object> overview() {
        ...
        data.put("accountCount", openAccountService.selectCount(new QueryWrapper<>()));
        data.put("appCount", openAppService.selectCount(new QueryWrapper<>()));
        data.put("identityCount", openIdentityService.selectCount(new QueryWrapper<>()));
        data.put("unionCount", openUnionService.selectCount(new QueryWrapper<>()));
        data.put("codeCount", openCodeService.selectCount(new QueryWrapper<>()));
        data.put("tokenCount", openTokenService.selectCount(new QueryWrapper<>()));
        data.put("activeAppCount", openAppService.selectCount(new QueryWrapper<OpenAppEntity>().eq("status", OpenAppEntity.STATUS_ACTIVE)));
        return data;
    }
    ...
    public PageUtils pageAccounts(Map<String, Object> params) {
        QueryWrapper<OpenAccountEntity> wrapper = new QueryWrapper<>();
        AdminPageQueries.applyKeyword(wrapper, params, "name", "uuid", "user");
        PageUtils page = openAccountService.queryPage(openAccountService.getPage(params, "create"), wrapper);
        enrichAccountRows(page);
        return page;
    }

    public PageUtils pageApps(Map<String, Object> params) {
        QueryWrapper<OpenAppEntity> wrapper = new QueryWrapper<>();
        ...
        PageUtils page = openAppService.queryPage(openAppService.getPage(params, "create"), wrapper);
        enrichAppRows(page);
        return page;
    }
    ...
    public PageUtils pageAppUsers(Map<String, Object> params) {
        ...
        OpenAppEntity app = requireApp(appId);
        QueryWrapper<OpenIdentityEntity> wrapper = new QueryWrapper<>();
        wrapper.eq("app_id", appId.trim());
        AdminPageQueries.applyKeyword(wrapper, params, "open_id", "user");
        PageUtils page = openIdentityService.queryPage(openIdentityService.getPage(params, "create"), wrapper);
        enrichIdentityRows(page, app.getAccount());
        return page;
    }

    public PageUtils pageUnions(Map<String, Object> params) {
        QueryWrapper<OpenUnionEntity> wrapper = new QueryWrapper<>();
        String account = AdminPageQueries.stringParam(params, "account");
        if (StringUtils.isNotBlank(account)) {
            wrapper.eq("account", account.trim());
        }
        AdminPageQueries.applyKeyword(wrapper, params, "union_id", "user");
        PageUtils page = openUnionService.queryPage(openUnionService.getPage(params, "create"), wrapper);
        enrichUnionRows(page);
        return page;
    }

    public PageUtils pageCodes(Map<String, Object> params) {
        QueryWrapper<OpenCodeEntity> wrapper = new QueryWrapper<>();
        String appId = AdminPageQueries.stringParam(params, "appId");
        if (StringUtils.isNotBlank(appId)) {
            wrapper.eq("app_id", appId.trim());
        }
        AdminPageQueries.applyKeyword(wrapper, params, "code", "user");
        PageUtils page = openCodeService.queryPage(openCodeService.getPage(params, "create"), wrapper);
        enrichSimpleUserRows(page);
        return page;
    }

    public PageUtils pageTokens(Map<String, Object> params) {
        QueryWrapper<OpenTokenEntity> wrapper = new QueryWrapper<>();
        String appId = AdminPageQueries.stringParam(params, "appId");
        if (StringUtils.isNotBlank(appId)) {
            wrapper.eq("app_id", appId.trim());
        }
        AdminPageQueries.applyKeyword(wrapper, params, "access_token", "refresh_token", "open_id", "union_id", "user");
        PageUtils page = openTokenService.queryPage(openTokenService.getPage(params, "update_time"), wrapper);
        enrichSimpleUserRows(page);
        return page;
    }
    ...
    private void enrichAccountRows(PageUtils page) {
        ...
        for (Object item : page.getList()) {
            ...
            row.put("appCount", openAppService.selectCount(new QueryWrapper<OpenAppEntity>().eq("account", account.getUuid())));
            row.put("unionCount", openUnionService.selectCount(new QueryWrapper<OpenUnionEntity>().eq("account", account.getUuid())));
            rows.add(row);
        }
        page.setList(rows);
    }
    ...
    private OplAppAdminView toAppView(OpenAppEntity app) {
        ...
        view.setUserCount(openIdentityService.selectCount(new QueryWrapper<OpenIdentityEntity>().eq("app_id", app.getAppId())));
        view.setCodeCount(openCodeService.selectCount(new QueryWrapper<OpenCodeEntity>().eq("app_id", app.getAppId())));
        view.setTokenCount(openTokenService.selectCount(new QueryWrapper<OpenTokenEntity>().eq("app_id", app.getAppId())));
        return view;
    }
