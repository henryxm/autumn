...
public class LanguageService extends LanguageServiceGen implements LoadFactory.Load, LoadFactory.Must, RefreshFactory.Refresh, CategoryHandler {
    ...
    public String[][] getLanguageItems() {
        return new String[][]{
                ...
                {"sys_string_table_scan", "全表查询", "Cross-table value scan"},
                ...
                {"sys_string_wall_management", "防火墙", "Firewall"},
                {"sys_string_session_management", "会话管理", "Session management"},
                {"sys_string_redis_management", "Redis管理", "Redis management"},
                {"sys_string_thread_management", "线程池管理", "Thread pool management"},
                {"sys_string_function_queue_management", "函数队列管理", "Function queue management"},
                ...
                {"sys_string_loading_title", "系统正在准备中", "System is preparing"},
                {"sys_string_loading_description", "系统正在后台完成启动检查，页面不会自动整页刷新；就绪后将自动进入目标页。", "Startup checks run in the background without full page reloads. You will enter the target page when ready."},
                {"sys_string_loading_retry_now", "立即检测", "Check now"},
                {"sys_string_loading_retrying", "正在重试，剩余", "Retrying in"},
                {"sys_string_loading_waiting", "已等待", "Waited"},
                {"sys_string_loading_checking", "正在后台检测服务就绪状态…", "Checking service readiness in the background…"},
                {"sys_string_loading_force_enter", "仍要进入", "Enter anyway"},
                {"sys_string_loading_failed", "启动失败，请查看服务日志或稍后重试", "Startup failed. Check server logs or retry later."},
                {"sys_string_loading_seconds", "秒", "s"},
                {"sys_string_loading_status", "当前状态", "Current status"},
                {"sys_string_loading_target", "目标页面", "Target page"},
                {"sys_string_loading_back_home", "返回首页", "Back home"},
                {"sys_string_loading_stage_bootstrap", "正在初始化核心服务", "Initializing core services"},
                {"sys_string_loading_stage_resource", "正在加载页面资源", "Loading page resources"},
                {"sys_string_loading_stage_check", "正在进行就绪检查", "Running readiness checks"},
                {"sys_string_loading_stage_ready", "服务已就绪，正在进入页面", "Service is ready, entering page"},
                {"sys_string_loading_brand", "秋枫系统", "Autumn Platform"},
