...
public class ScheduleJobLogServiceGen extends ServiceImpl<ScheduleJobLogDao, ScheduleJobLogEntity> implements InitFactory.Init {
    ...
    */
    ...
    public String[][] getMenus() {
        String menuKey = SysMenuService.getMenuKey("Job", "ScheduleJobLog");
        String[][] menus = new String[][]{
                //{0:菜单名字,1:URL,2:权限,3:菜单类型,4:ICON,5:排序,6:MenuKey,7:ParentKey,8:Language}
                {"任务日志", "modules/job/schedulejoblog", "job:schedulejoblog:list,job:schedulejoblog:info,job:schedulejoblog:save,job:schedulejoblog:update,job:schedulejoblog:delete", "1", "fa " + ico(), order(), menuKey, parentMenu(), "job_schedulejoblog_table_comment"},
                {"查看", null, "job:schedulejoblog:list,job:schedulejoblog:info", "2", null, order(), SysMenuService.getMenuKey("Job", "ScheduleJobLogInfo"), menuKey, "sys_string_lookup"},
                {"新增", null, "job:schedulejoblog:save", "2", null, order(), SysMenuService.getMenuKey("Job", "ScheduleJobLogSave"), menuKey, "sys_string_add"},
                {"修改", null, "job:schedulejoblog:update", "2", null, order(), SysMenuService.getMenuKey("Job", "ScheduleJobLogUpdate"), menuKey, "sys_string_change"},
                {"删除", null, "job:schedulejoblog:delete", "2", null, order(), SysMenuService.getMenuKey("Job", "ScheduleJobLogDelete"), menuKey, "sys_string_delete"},
        };
        return menus;
    }
}
