...
public class SysConfigService extends ServiceImpl<SysConfigDao, SysConfigEntity> implements LoopJob.Job, LoopJob.OneMinute, HostFactory.Host, InitFactory.Init {
    ...
    public String[][] getLanguageItems() {
        String[][] items = new String[][]{
                ...
                {config_lang_prefix + "site_ssl_name", "传输加密", "Transmission encryption"},
                {config_lang_prefix + "site_ssl_description", "站点是否支持SSL加密传输", "Whether the site supports SSL encrypted transmission?"},
                {config_lang_prefix + "cluster_root_domain_name", "集群根域名", "Cluster Root Domain"},
                {config_lang_prefix + "cluster_root_domain_description", "集群的根域名，当开启Redis后，有相同根域名后缀的服务会使用相同的Cookie", "The root domain name of the cluster. When Redis is enabled, services with the same root domain name suffix will use the same cookie"},
    ...
    public String[][] getConfigItems() {
        String[][] mapping = new String[][]{
                ...
                {SUPER_PASSWORD, getSuperPassword(), "0", "系统的超级密码，使用该密码可以登录任何账户，如果为空或小于20位，表示禁用该密码", sys_config, string_type, config_lang_prefix + "super_password_name", config_lang_prefix + "super_password_description"},
                ...
                {LOGIN_AUTHENTICATION, "oauth2:" + getClientId(), "0", "系统登录授权，参数类型：①:localhost; ②:oauth2:" + getClientId() + "; ③shell:" + getClientId(), sys_config, selection_type, config_lang_prefix + "login_authentication_name", config_lang_prefix + "login_authentication_description", "localhost,oauth2:" + getClientId() + ",shell:" + getClientId()},
                ...
                {CLUSTER_ROOT_DOMAIN, getClusterRootDomain(), "1", "集群的根域名，当开启Redis后，有相同根域名后缀的服务会使用相同的Cookie，集群可通过Cookie中的登录用户进行用户同步", sys_config, string_type, config_lang_prefix + "cluster_root_domain_name", config_lang_prefix + "cluster_root_domain_description"},
