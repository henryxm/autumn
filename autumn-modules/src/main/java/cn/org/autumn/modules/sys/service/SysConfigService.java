...
public class SysConfigService extends ServiceImpl<SysConfigDao, SysConfigEntity> implements LoopJob.ThirtyMinute, LoopJob.OneMinute, HostFactory.Host, InitFactory.Init, InitFactory.After, CategoryHandler, DomainHandler, ClearHandler {
    ...
    public String[][] getConfigItems() {
        return new String[][]{
                ...
                {DISTRIBUTED_LOCK_CONFIG, GsonConfig.getGson().toJson(new DistributedLockConfig()), "1", "分布式锁开关与参数配置", config, json_type, DistributedLockConfig.class.getName()},
                ...
                {AUTH_SITE_CONFIG, GsonConfig.getGson().toJson(new AuthSiteConfig()), "1", "认证站点角色与联邦登录配置", config, json_type, AuthSiteConfig.class.getName()},
                {SITE_PORTAL_CONFIG, GsonConfig.getGson().toJson(new SitePortalConfig()), "1", "登录页品牌、版权、备案与法律链接", config, json_type, SitePortalConfig.class.getName()},
        };
    }
    ...
    public SitePortalConfig updateSitePortalConfig(SitePortalConfig config) {
        ...
        updateValueByKey(SITE_PORTAL_CONFIG, GsonConfig.getGson().toJson(toSave));
        return toSave;
    }
    ...
    public LoadingTheme updateLoadingTheme(LoadingTheme theme) {
        ...
        toSave.normalize();
        updateValueByKey(LOADING_THEME, GsonConfig.getGson().toJson(toSave));
        return toSave;
    }
