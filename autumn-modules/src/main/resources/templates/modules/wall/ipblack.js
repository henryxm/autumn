$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '${lang.wall_ipblack_column_create_time}', name: 'createTime', index: 'create_time', width: 80 }, 
			{ label: '${lang.wall_ipblack_column_update_time}', name: 'updateTime', index: 'update_time', width: 80 }, 
        ],
		viewrecords: true,
        height: 385,
        rowNum: 10,
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.ipBlack.id == null ? "wall/ipblack/save" : "wall/ipblack/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.ipBlack),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('${lang.sys_string_are_sure_to_delete}？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "wall/ipblack/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "wall/ipblack/info/"+id, function(r){
                vm.ipBlack = r.ipBlack;
            });
		},
