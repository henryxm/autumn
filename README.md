...
- 当前版本：`2.0.0`
- 技术基座：**Spring Boot 2.7.x**、MyBatis-Plus 2.x、Shiro、Redis、EhCache、FreeMarker（前端页面以模块内静态资源为准）
- 设计目标：
...
- **Autumn 2.0.0（master，JDK 8 + Spring Boot 2.7.18）**：`.cursor/skills/autumn-framework-2x/SKILL.md`  
  - 与上述 `docs/` 路径一致；可同步到个人 `~/.cursor/skills`。
- **Autumn 3.0.0（3.0.0 分支，JDK 17+ + Spring Boot 3.5.x）**：在将 **master 合并进 3.0.0** 后，于该分支维护 **`.cursor/skills/autumn-framework-3x/SKILL.md`**（与 2.x 区分，勿混用）。
- 业务工程建议在 `AGENTS.md` 或首轮提示中写明当前依赖的 **Autumn 主版本**，便于选用 **2x** 或 **3x** Skill。
...
├─ autumn-starter    # 一站式引入 Autumn 框架
